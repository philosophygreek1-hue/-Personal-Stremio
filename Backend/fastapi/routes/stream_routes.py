...
async def get_manifest(token: str, token_data: dict = Depends(verify_token)):
    ...
    return {
        ...
        "description": "Stream your personal Telegram files.",
        "types": ["movie"],
        "resources": resources,
        "catalogs": catalogs,
        "idPrefixes": ["ps"],
...
<style>
  ...
  button {{ cursor: pointer; padding: 10px 20px; background: #7b68ee;
...
@router.get("/{token}/catalog/movie/{id}/{extra:path}.json")
@router.get("/{token}/catalog/movie/{id}.json")
...
@router.get("/{token}/meta/movie/{id}.json")
...
@router.get("/{token}/stream/movie/{id}.json")
