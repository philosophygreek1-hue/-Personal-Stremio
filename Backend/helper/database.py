...
class Database:
    ...
    # ─────────────────────────────────────────────────────────
    #  Personal Files (no TMDB — folder-based)
    # ─────────────────────────────────────────────────────────

    def _personal_db(self):
        """Always use storage_1 for personal files."""
        return self.dbs["storage_1"]["personal_files"]

    async def insert_personal_file(
        self,
        metadata_info: dict,
        channel: int,
        msg_id: int,
        ...
        """Insert or update a personal file entry."""
        try:
            col = self._personal_db()
            personal_id = metadata_info.get("personal_id") or metadata_info.get("imdb_id")
            encoded_string = metadata_info.get("encoded_string")
            quality = metadata_info.get("quality", "File")
            ...
            existing = await col.find_one({"personal_id": personal_id})
            if existing:
                # Add new quality entry if not duplicate
                existing_ids = [q.get("id") for q in existing.get("telegram", [])]
                if encoded_string not in existing_ids:
                    await col.update_one(
                        {"personal_id": personal_id},
                        {
                            "$push": {"telegram": quality_entry},
                ...
                doc = {
                    ...
                    "folder": metadata_info.get("folder", "General"),
                    "description": metadata_info.get("description", ""),
                    "poster": metadata_info.get("poster", ""),
                    "backdrop": metadata_info.get("backdrop", ""),
                    "logo": metadata_info.get("logo", ""),
                    "genres": metadata_info.get("genres", []),
                    "release_year": metadata_info.get("year"),
                    "rating": 0,
                    "media_type": "personal",
                    "db_index": 1,
                    "telegram": [quality_entry],
                    "updated_on": datetime.utcnow(),
                }
                await col.insert_one(doc)
                return personal_id
        except Exception as e:
            LOGGER.error(f"insert_personal_file error: {e}")
            return None

    async def get_personal_file(self, personal_id: str) -> Optional[dict]:
        try:
            col = self._personal_db()
            doc = await col.find_one({"personal_id": personal_id})
            return convert_objectid_to_str(doc) if doc else None
        except Exception as e:
            LOGGER.error(f"get_personal_file error: {e}")
            return None

    async def sort_personal_files(
        self,
        sort: list,
        page: int = 1,
        page_size: int = 15,
        ...
        try:
            ...
            sort_list = [(k, 1 if v == "asc" else -1) for k, v in sort]
            skip = (page - 1) * page_size
            cursor = col.find(query).sort(sort_list).skip(skip).limit(page_size)
            docs = await cursor.to_list(None)
            return {"files": [convert_objectid_to_str(d) for d in docs]}
        except Exception as e:
            LOGGER.error(f"sort_personal_files error: {e}")
            return {"files": []}
    ...
    async def search_personal_files(
        ...
        try:
            ...
            mongo_query = {"$or": [{"title": regex}, {"folder": regex}]}
            skip = (page - 1) * page_size
            cursor = col.find(mongo_query).skip(skip).limit(page_size)
            docs = await cursor.to_list(None)
            return {"results": [convert_objectid_to_str(d) for d in docs]}
        except Exception as e:
            LOGGER.error(f"search_personal_files error: {e}")
            return {"results": []}

    async def get_all_folders(self) -> list:
        try:
            col = self._personal_db()
            folders = await col.distinct("folder")
            return sorted([f for f in folders if f])
        except Exception as e:
            LOGGER.error(f"get_all_folders error: {e}")
            return []

    async def delete_personal_file(self, personal_id: str) -> bool:
        try:
            col = self._personal_db()
            result = await col.delete_one({"personal_id": personal_id})
            return result.deleted_count > 0
        except Exception as e:
            LOGGER.error(f"delete_personal_file error: {e}")
            return False
