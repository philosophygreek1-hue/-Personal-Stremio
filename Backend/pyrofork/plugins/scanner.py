...
async def _scan_channel(client: Client, chat_id: int):
    ...
    while empty_streak < MAX_EMPTY_BATCHES and current < MAX_ID_CAP:
        ...
        for message in messages:
            ...
            # Accept all file types for personal files addon
            has_file = bool(message.video or message.document or message.audio)
            if not has_file:
                s.skipped_nonvid += 1
                s.processed += 1
            ...
            title = message.caption or getattr(file, 'file_name', None) or getattr(file, 'title', None) or "file"
            msg_id = message.id
            ...
            # Keep original file extension for personal files
            # ── Insert into DB ───────────────────────────────────
            try:
